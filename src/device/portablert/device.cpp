...
bool device_portablert_init()
{
  ...
  if (cuew_result == CUEW_SUCCESS) {
    ...
    if (PortableRTDevice::have_precompiled_kernels()) {
...
{
#ifdef WITH_PORTABLERT
  return make_unique<PortableRTDevice>(info, stats, profiler, headless);
#else
  (void)info;
  (void)stats;
  (void)profiler;
...
#ifdef WITH_PORTABLERT
static CUresult device_portablert_safe_init()
{
#  ifdef _WIN32
  __try
  {
...
{
#ifdef WITH_PORTABLERT
  CUresult result = device_portablert_safe_init();
