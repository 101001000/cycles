...
bool PortableRTDevice::have_precompiled_kernels()
{
  string cubins_path = path_get("lib");
  return path_exists(cubins_path);
}

BVHLayoutMask PortableRTDevice::get_bvh_layout_mask(uint /*kernel_features*/) const
{
  return BVH_LAYOUT_BVH2;
}

void PortableRTDevice::set_error(const string &error)
{
...
PortableRTDevice::PortableRTDevice(const DeviceInfo &info, Stats &stats, Profiler &profiler, bool headless)
...
PortableRTDevice::~PortableRTDevice()
{
  texture_info.free();
  if (cuModule) {
...
bool PortableRTDevice::support_device(const uint /*kernel_features*/)
{
  int major, minor;
  cuDeviceGetAttribute(&major, CU_DEVICE_ATTRIBUTE_COMPUTE_CAPABILITY_MAJOR, cuDevId);
  cuDeviceGetAttribute(&minor, CU_DEVICE_ATTRIBUTE_COMPUTE_CAPABILITY_MINOR, cuDevId);
...
bool PortableRTDevice::check_peer_access(Device *peer_device)
{
  if (peer_device == this) {
    return false;
  }
  if (peer_device->info.type != DEVICE_PORTABLERT) {
    return false;
  }

  PortableRTDevice *const peer_device_portablert = static_cast<PortableRTDevice *>(peer_device);

  int can_access = 0;
  cuda_assert(cuDeviceCanAccessPeer(&can_access, cuDevice, peer_device_portablert->cuDevice));
  if (can_access == 0) {
    return false;
  }
  ...
  cuda_assert(cuDeviceGetP2PAttribute(&can_access,
                                      ...
                                      cuDevice,
                                      peer_device_portablert->cuDevice));
  if (can_access == 0) {
    return false;
  }
  ...
  {
    const PortableRTContextScope scope(this);
    CUresult result = cuCtxEnablePeerAccess(peer_device_portablert->cuContext, 0);
  ...
  {
    const PortableRTContextScope scope(peer_device_portablert);
...
bool PortableRTDevice::use_adaptive_compilation()
{
 ...
 */
string PortableRTDevice::compile_kernel_get_common_cflags(const uint kernel_features)
{
...
string PortableRTDevice::compile_kernel(const string &common_cflags,
                                  const char *name,
                                  const char *base,
                                  bool force_ptx)
{
...
bool PortableRTDevice::load_kernels(const uint kernel_features)
{
  ...
  /* open module */
  PortableRTContextScope scope(this);
...
void PortableRTDevice::reserve_local_memory(const uint kernel_features)
{
...

  {
    ...
    PortableRTDeviceQueue queue(this);
...
void PortableRTDevice::get_device_memory_info(size_t &total, size_t &free)
{
  PortableRTContextScope scope(this);
...
bool PortableRTDevice::alloc_device(void *&device_pointer, const size_t size)
{
  PortableRTContextScope scope(this);
...
void PortableRTDevice::free_device(void *device_pointer)
{
  PortableRTContextScope scope(this);
...
bool PortableRTDevice::shared_alloc(void *&shared_pointer, const size_t size)
{
  PortableRTContextScope scope(this);
...
void PortableRTDevice::shared_free(void *shared_pointer)
{
  PortableRTContextScope scope(this);
...
void *PortableRTDevice::shared_to_device_pointer(const void *shared_pointer)
{
  PortableRTContextScope scope(this);
...
void PortableRTDevice::copy_host_to_device(void *device_pointer, void *host_pointer, const size_t size)
{
  const PortableRTContextScope scope(this);
...
void PortableRTDevice::mem_alloc(device_memory &mem)
{
...
void PortableRTDevice::mem_copy_to(device_memory &mem)
{
...
void PortableRTDevice::mem_move_to_host(device_memory &mem)
{
  if (mem.type == MEM_GLOBAL) {
    global_free(mem);
    global_alloc(mem);
...
void PortableRTDevice::mem_copy_from(
...
void PortableRTDevice::mem_zero(device_memory &mem)
{
  if (!mem.device_pointer) {
    mem_alloc(mem);
  }
...
void PortableRTDevice::mem_free(device_memory &mem)
{
  if (mem.type == MEM_GLOBAL) {
    global_free(mem);
  }
...
device_ptr PortableRTDevice::mem_alloc_sub_ptr(device_memory &mem, const size_t offset, size_t /*size*/)
{
  return (device_ptr)(((char *)mem.device_pointer) + mem.memory_elements_size(offset));
}

void PortableRTDevice::const_copy_to(const char *name, void *host, const size_t size)
{
  PortableRTContextScope scope(this);
  CUdeviceptr mem;
  size_t bytes;
...
void PortableRTDevice::global_alloc(device_memory &mem)
{
...
void PortableRTDevice::global_copy_to(device_memory &mem)
{
  if (!mem.device_pointer) {
    generic_alloc(mem);
    generic_copy_to(mem);
...
void PortableRTDevice::global_free(device_memory &mem)
{
...
void PortableRTDevice::tex_alloc(device_texture &mem)
{
  PortableRTContextScope scope(this);
...
void PortableRTDevice::tex_copy_to(device_texture &mem)
{
  if (!mem.device_pointer) {
...
void PortableRTDevice::tex_free(device_texture &mem)
{
  PortableRTContextScope scope(this);
  thread_scoped_lock lock(device_mem_map_mutex);
...
unique_ptr<DeviceQueue> PortableRTDevice::gpu_queue_create()
{
  return make_unique<PortableRTDeviceQueue>(this);
}

bool PortableRTDevice::should_use_graphics_interop()
{
...
int PortableRTDevice::get_num_multiprocessors()
{
  return get_device_default_attribute(CU_DEVICE_ATTRIBUTE_MULTIPROCESSOR_COUNT, 0);
}

int PortableRTDevice::get_max_num_threads_per_multiprocessor()
{
  return get_device_default_attribute(CU_DEVICE_ATTRIBUTE_MAX_THREADS_PER_MULTIPROCESSOR, 0);
}

bool PortableRTDevice::get_device_attribute(CUdevice_attribute attribute, int *value)
{
  PortableRTContextScope scope(this);
...
int PortableRTDevice::get_device_default_attribute(CUdevice_attribute attribute, const int default_value)
{
  int value = 0;
