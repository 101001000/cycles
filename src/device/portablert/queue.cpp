...
#  include "device/portablert/queue.h"

#  include "device/portablert/device_impl.h"
#  include "device/portablert/graphics_interop.h"
#  include "device/portablert/kernel.h"

CCL_NAMESPACE_BEGIN

/* PortableRTDeviceQueue */

PortableRTDeviceQueue::PortableRTDeviceQueue(PortableRTDevice *device)
    : DeviceQueue(device), cuda_device_(device), cuda_stream_(nullptr)
{
  const PortableRTContextScope scope(cuda_device_);
  cuda_device_assert(cuda_device_, cuStreamCreate(&cuda_stream_, CU_STREAM_NON_BLOCKING));
}

PortableRTDeviceQueue::~PortableRTDeviceQueue()
{
  const PortableRTContextScope scope(cuda_device_);
  cuStreamDestroy(cuda_stream_);
}

int PortableRTDeviceQueue::num_concurrent_states(const size_t state_size) const
{
...
int PortableRTDeviceQueue::num_concurrent_busy_states(const size_t /*state_size*/) const
{
  const int max_num_threads = cuda_device_->get_num_multiprocessors() *
                              cuda_device_->get_max_num_threads_per_multiprocessor();
...
void PortableRTDeviceQueue::init_execution()
{
...
bool PortableRTDeviceQueue::enqueue(DeviceKernel kernel,
                              const int work_size,
                              const DeviceKernelArguments &args)
{
  ...
  const PortableRTDeviceKernel &cuda_kernel = cuda_device_->kernels.get(kernel);
...
bool PortableRTDeviceQueue::synchronize()
{
  if (cuda_device_->have_error()) {
    return false;
  }
...
void PortableRTDeviceQueue::zero_to_device(device_memory &mem)
{
...
void PortableRTDeviceQueue::copy_to_device(device_memory &mem)
{
  assert(mem.type != MEM_GLOBAL && mem.type != MEM_TEXTURE);
...
void PortableRTDeviceQueue::copy_from_device(device_memory &mem)
{
  assert(mem.type != MEM_GLOBAL && mem.type != MEM_TEXTURE);
...
void PortableRTDeviceQueue::assert_success(CUresult result, const char *operation)
{
...
unique_ptr<DeviceGraphicsInterop> PortableRTDeviceQueue::graphics_interop_create()
{
  return make_unique<PortableRTDeviceGraphicsInterop>(this);
}
